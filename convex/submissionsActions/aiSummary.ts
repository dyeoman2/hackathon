    ...
    try {
      ...
      if (!indexed) {
        ...
        if (args.attempt < maxAttempts) {
          console.log(
            `[AI Search] ⏳ Not indexed yet - rescheduling check in ${pollIntervalMs}ms (attempt ${args.attempt + 1}/${maxAttempts})`,
          );
          await ctx.scheduler.runAfter(
            pollIntervalMs,
            (
        ...
        const completedAt = Date.now();
        console.log(
          `[AI Search] ✅ Files confirmed indexed - recording sync completion time: ${new Date(completedAt).toISOString()}`,
        );
        await ctx.runMutation(
          (
            internal.submissions as unknown as {
